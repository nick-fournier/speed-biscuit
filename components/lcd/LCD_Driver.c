...
// Function to read the LCD ID
uint8_t LCD_Read_Id(void) {
    uint8_t reg = 0xDC;
    uint8_t rx_val;

    DEV_Digital_Write(LCD_CS_PIN, 0); // Assert CS
    DEV_Digital_Write(LCD_DC_PIN, 0); // Command mode

    SPI4W_Write_Byte(reg);            // Send register address

    // Read the response from the register
    rx_val = SPI4W_Read_Byte(reg);

    DEV_Digital_Write(LCD_CS_PIN, 1); // Deassert CS
